...
const MEMORY_NODES: MemoryNode[] = [
  { id: "rn",       label: "React Native",  category: "skill",       x: 48, y: 36, r: 20, desc: "98/100 · Bridgeless, JSI, TurboModules, 60fps on $150 Android" },
  { id: "ts",       label: "TypeScript",    category: "skill",       x: 24, y: 58, r: 13, desc: "96/100 · Strict mode, generics, discriminated unions" },
  { id: "ai",       label: "AI Systems",    category: "skill",       x: 74, y: 52, r: 17, desc: "88/100 · HIPAA RAG, MediaPipe CV, LLM orchestration" },
  { id: "web3",     label: "Web3 / DeFi",  category: "skill",       x: 30, y: 22, r: 12, desc: "85/100 · Solidity, Ethereum, Wagmi v2, on-chain systems" },
  { id: "lead",     label: "Leadership",   category: "skill",       x: 70, y: 24, r: 14, desc: "0→21 engineers · 3 promoted to senior at Synapsis" },
  { id: "dau",      label: "50K DAU",      category: "achievement", x: 18, y: 40, r: 10, desc: "Peak daily active users · Vulcan Eleven fantasy sports" },
  { id: "synapsis", label: "Synapsis '25", category: "milestone",   x: 56, y: 68, r: 11, desc: "Founding Engineer · 5 apps in 13 months · HIPAA AI" },
  { id: "nonce",    label: "NonceBlox",    category: "milestone",   x: 18, y: 76, r: 10, desc: "2021–2025 · 13+ apps · Web3 Lead · FinTech / Gaming" },
  { id: "bca",      label: "BCA 2017",     category: "milestone",   x: 80, y: 72, r: 9,  desc: "The Heritage Academy Kolkata · Where it all began" },
  { id: "kolkata",  label: "Kolkata",      category: "milestone",   x: 38, y: 82, r: 9,  desc: "Kolkata, India · Remote exclusively · IST (UTC+5:30)" },
];
...
const PortraitCard = memo(function PortraitCard() {
  return (
    <div style={{ position: "absolute", inset: 0 }}>
      ...
      {/* Purple ambient tint — top-left light leak */}
      <div
        aria-hidden
        style={{
          position: "absolute", inset: 0,
          background: "linear-gradient(155deg, rgba(124,58,237,0.22) 0%, transparent 55%)",
      ...
      <div
        ...
        style={{
          ...
          background: `linear-gradient(to bottom,
            rgba(0,0,0,0) 0%, rgba(0,0,0,0.01) 28%,
            rgba(6,2,20,0.18) 46%, rgba(6,2,20,0.62) 64%,
            rgba(6,2,20,0.90) 80%, rgba(6,2,20,0.98) 100%)`,
          pointerEvents: "none",
        }}
      />
      ...
      <div style={{
        ...
        background: "rgba(255,255,255,0.08)", backdropFilter: "blur(14px) saturate(1.8)",
        WebkitBackdropFilter: "blur(14px) saturate(1.8)",
        border: "0.5px solid rgba(255,255,255,0.14)", borderRadius: 999,
        padding: "4px 11px", fontFamily: "var(--font-mono)", fontSize: 9,
      ...
      <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: "20px 20px 22px" }}>
        <div style={{
          ...
          <span style={{ fontFamily: "var(--font-mono)", fontSize: 9.5, color: "#22C55E", letterSpacing: "0.04em" }}>
            Available · Remote
          </span>
        </div>
        ...
        <div style={{ fontFamily: "var(--font-mono)", fontSize: 10, color: "rgba(155,120,255,0.85)", marginTop: 4, letterSpacing: "0.03em" }}>
          ...
          {[
            ...
            <a
              ...
              style={{
                ...
                textDecoration: "none", color: "rgba(255,255,255,0.55)", transition: "background 0.2s", flexShrink: 0,
              }}
            >
              <Icon />
            </a>
...
/* ─────────────────────────────────────────────────────────
   Card 1: Memory Constellation — named nodes, hoverable + clickable
   ───────────────────────────────────────────────────────── */
const MemoryCard = memo(function MemoryCard() {
  ...
  const activeNode = activeId ? MEMORY_NODES.find((n) => n.id === activeId) : null;

  /* Find node positions for SVG edge lookup */
  const nodeMap = new Map(MEMORY_NODES.map((n) => [n.id, n]));
  ...
  return (
    <div style={{ position: "absolute", inset: 0, overflow: "hidden", background: "#F0EEFF" }}>
        ...
        {EDGES.map(([aId, bId]) => {
          ...
          return (
            <line
              ...
              stroke={isActive ? "rgba(124,58,237,0.45)" : "rgba(124,58,237,0.14)"}
              strokeWidth={isActive ? "0.35" : "0.20"}
      ...
      {MEMORY_NODES.map((node) => {
        ...
        const labelShort = node.label.length > 12 ? node.label.slice(0, 11) + "…" : node.label;
        ...
        return (
          <button
            ...
            style={{
              ...
              boxShadow: isActive
                ? `0 0 0 ${node.r * 0.6}px ${color}15, 0 0 ${node.r * 1.2}px ${color}30`
                : "none",
              cursor: "pointer",
              padding: 0,
              transition: "all 0.22s cubic-bezier(0.34,1.56,0.64,1)",
              zIndex: isActive ? 4 : 2,
              animation: isActive ? "none" : `pm-nodepulse ${2.8 + MEMORY_NODES.indexOf(node) * 0.3}s ease-in-out infinite`,
            }}
          >
            {/* Central dot */}
            <span style={{
              ...
              <span style={{
                width: Math.max(3, node.r * 0.3),
                height: Math.max(3, node.r * 0.3),
                borderRadius: "50%",
                background: color,
            ...
            {/* Label (visible when active or on large nodes) */}
            {(isActive || node.r >= 14) && (
              <span style={{
                position: "absolute",
                top: "110%",
      ...
      {/* Tooltip — appears when a node is selected or hovered */}
      {activeNode && (
        <div
          aria-live="polite"
          style={{
        ...
        >
          ...
          <div style={{
            ...
            color: "#1a1a2e", lineHeight: 1.3, marginBottom: 4,
          }}>
            {activeNode.label}
          </div>
          <div style={{
            fontFamily: "var(--font-mono)", fontSize: 9, color: "rgba(0,0,0,0.5)", lineHeight: 1.5,
          }}>
            {activeNode.desc}
          </div>
        </div>
      ...
      <div style={{
        ...
        padding: "7px 16px", fontFamily: "var(--font-mono)", fontSize: 9.5,
...
/* ─────────────────────────────────────────────────────────
   Card 2: Agent Context — mem0-styled query result
   ───────────────────────────────────────────────────────── */
const AgentContextCard = memo(function AgentContextCard() {
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => {
      i++;
      setVisibleCount(i);
      if (i >= AGENT_MEMORIES.length) clearInterval(t);
    }, 320);
    ...
    <div style={{ position: "absolute", inset: 0, background: "#0C0C14", overflow: "hidden" }}>
      ...
      <div style={{ position: "absolute", inset: 0, padding: "18px 18px 16px", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ marginBottom: 14, display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <span style={{ fontFamily: "var(--font-mono)", fontSize: 8.5, color: "rgba(255,255,255,0.25)", flexShrink: 0 }}>
            confidence
          </span>
        </div>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 6, overflow: "hidden" }}>
          {AGENT_MEMORIES.map((mem, i) => (
            ...
            >
              ...
              <span style={{ fontFamily: "var(--font-mono)", fontSize: 9.5, color: "rgba(255,255,255,0.55)", lineHeight: 1.5 }}>
                {mem}
              </span>
            </div>
          ))}
        ...
        <div style={{
          ...
          <span style={{ fontFamily: "var(--font-mono)", fontSize: 8.5, color: "rgba(124,58,237,0.45)", letterSpacing: "0.04em" }}>
            user_id: amit_chakraborty
          </span>
          <span style={{
            display: "inline-flex", alignItems: "center", gap: 4,
            fontFamily: "var(--font-mono)", fontSize: 8.5, color: "rgba(34,197,94,0.70)",
          }}>
            <span style={{
              width: 5, height: 5, borderRadius: "50%", background: "#22C55E",
...
/* Card array — order matches initial state [back=StatsCard/AgentContext, mid=MemoryCard, front=PortraitCard] */
        ...
        {[0, 1, 2].map((cardIdx) => {
          ...
          return (
            <button
              ...
              style={{
                width: isCurrentFront ? 20 : 6, height: 6, borderRadius: 999,
...
export function Hero() {
  ...
  useEffect(() => {
    ...
    import("gsap")
      .then(({ gsap }) => {
        ...
        gsap.fromTo(items, { opacity: 0, y: 14 }, { opacity: 1, y: 0, duration: 0.65, stagger: 0.08, ease: "power3.out" });
      })
      .catch(() => {
  ...
  return (
    ...
    >
      {/* Very subtle purple tint top-right — zero orbs */}
      <div
        aria-hidden
        style={{
          position: "absolute", top: 0, right: 0, width: "50%", height: "70%",
          background: "radial-gradient(ellipse 70% 60% at 80% 10%, rgba(124,58,237,0.045) 0%, transparent 75%)",
          pointerEvents: "none",
        }}
      />
      ...
      >
        ...
        <div ref={textRef}>
          ...
          <div className="h-item" style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 26, opacity: 0 }}>
            <img
            ...
            <div style={{
              ...
              <span style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--signal)", letterSpacing: "0.02em" }}>
                Available · Remote
              </span>
            </div>
          </div>
          ...
          <p
            className="h-item"
            style={{ fontSize: 15, color: "var(--ink-secondary)", lineHeight: 1.68, marginBottom: 28, maxWidth: 400, opacity: 0 }}
          >
          ...
          {/* Trust badges */}
          <div className="h-item" style={{ display: "flex", alignItems: "center", marginBottom: 28, flexWrap: "wrap", gap: 0, opacity: 0 }}>
            {TRUST_BADGES.map((b, i) => (
              <div
                key={b.label}
                style={{
                  display: "flex", flexDirection: "column", gap: 2,
                  paddingRight: 18, paddingLeft: i === 0 ? 0 : 18,
              ...
              >
                <span style={{
                  ...
                  color: "var(--ink-primary)", fontWeight: 700,
                ...
                <span style={{
                  fontFamily: "var(--font-mono)", fontSize: 10,
                  color: "var(--ink-tertiary)", letterSpacing: "0.05em", textTransform: "uppercase",
                }}>
                  {b.label}
                </span>
              </div>
          ...
          <div className="h-item" style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 22, opacity: 0 }}>
            <a
              href="#projects"
              style={{
                background: "var(--signal)", color: "#fff", fontSize: 13.5, fontWeight: 500,
                padding: "11px 22px", borderRadius: 999, textDecoration: "none",
                display: "inline-block", letterSpacing: "-0.01em", transition: "opacity 0.2s",
              }}
            ...
            <a
              ...
              download
              style={{
                background: "var(--signal-light)", color: "var(--signal)", fontSize: 13.5, fontWeight: 500,
                padding: "11px 22px", borderRadius: 999, textDecoration: "none",
                display: "inline-block", letterSpacing: "-0.01em", transition: "opacity 0.2s",
              }}
            ...
            {[
              ...
              <a
                ...
                style={{
                  ...
                  color: "var(--ink-tertiary)", textDecoration: "none", transition: "color 0.15s",
                }}
              >
                <Icon />
              </a>
            ))}
            <span style={{ marginLeft: 8, fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--ink-quaternary)", letterSpacing: "0.01em" }}>
              amit@devamit.co.in
            </span>
          </div>
        </div>
